...
V2.0.1 是 V2.0.0 的小版本更新，重点增加 TF 卡 CSV 配置导入，并优化设置首页在 M5 Core 320x240 屏幕上的显示。

Windows 用户推荐下载 GitHub Release 中的：

- `fmo-companion-m5core-windows-flasher-v2.0.1.zip`

完整解压后运行 `flash_m5core.bat`，按提示输入设备管理器中看到的 COM 口即可刷写。

本版本新增：

- 设置页第一行 `TF卡配置文件`，可直接从 TF 卡根目录读取 `fmo_profiles.csv`；
- TF 卡建议使用 32GB 以下容量，并格式化为 FAT32；
- CSV 支持全局呼号和 5 组网络配置，适合在电脑上编辑后导入；
- 设置首页改为紧凑布局，避免底部 `返回 / 保存重启` 按钮被遮挡；
- `选择配置` 回到首页时固定显示当前已启用配置号。
- `选择配置` 中按中键直接启用完整配置，空配置可继续进入网络配置编辑；
- 配置详情页移除重复的“启用配置”入口，并重新调整 5 行布局；
- FMO 地址输入页改用 ASCII 标题和提示，避免中文缺字显示。
...
- 可以进入网络配置修改当前选中配置；
- 可以保存配置并启用为当前连接配置。
- 可以通过 TF 卡根目录 `fmo_profiles.csv` 一次导入全局呼号和 5 组配置。
...
1. 下载并完整解压 `fmo-companion-m5core-windows-flasher-v2.0.1.zip`。
2. 用 USB 连接 M5 Core。
